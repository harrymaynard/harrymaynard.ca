...
const main = function () {
    ...
    const simulator = new Simulation.Simulator(simulatorCanvas, window.innerWidth, window.innerHeight);

    // const profile = new UI.Profile(document.getElementById(Common.PROFILE_CANVAS_ID)),
    //     sizeSlider = new UI.Slider(cameraDiv, Common.SIZE_SLIDER_X, Common.SIZE_SLIDER_Z,
    ...
    //     choppinessSlider = new UI.Slider(cameraDiv, Common.CHOPPINESS_SLIDER_X, Common.CHOPPINESS_SLIDER_Z,
    ...
    const windArrow = new UI.Arrow(cameraDiv, Common.INITIAL_WIND[0], Common.INITIAL_WIND[1]);
    ...
    //     Common.premultiplyMatrix(inverseProjectionViewMatrix, viewMatrix, projectionMatrix);
    //     Common.invertMatrix(inverseProjectionViewMatrix, inverseProjectionViewMatrix);

    //     Common.setVector4(nearPoint, (x / width) * 2.0 - 1.0, ((height - y) / height) * 2.0 - 1.0, 1.0, 1.0);
    //     Common.transformVectorByMatrix(nearPoint, nearPoint, inverseProjectionViewMatrix);

    //     Common.setVector4(farPoint, (x / width) * 2.0 - 1.0, ((height - y) / height) * 2.0 - 1.0, -1.0, 1.0);
    //     Common.transformVectorByMatrix(farPoint, farPoint, inverseProjectionViewMatrix);

    //     Common.projectVector4(nearPoint, nearPoint);
    //     Common.projectVector4(farPoint, farPoint);
    ...
    //     const mousePosition = Common.getMousePosition(event, uiDiv);
    //     const mouseX = mousePosition.x,
    //         mouseY = mousePosition.y;

    //     const point = unproject(camera.getViewMatrix(), mouseX, mouseY, width, height);

    //     if (windArrow.distanceToTip(point) < Common.ARROW_TIP_RADIUS) {
    //         mode = Common.ROTATING;
    //     } else if (sizeSlider.distanceToHandle(point) < Common.SIZE_HANDLE_RADIUS) {
    //         mode = Common.SLIDING_SIZE;
    //     } else if (choppinessSlider.distanceToHandle(point) < Common.CHOPPINESS_HANDLE_RADIUS) {
    ...
    //     if (windArrow.distanceToTip(point) < Common.ARROW_TIP_RADIUS || mode === Common.ROTATING) {
    //         overlayDiv.style.cursor = 'move';
    //     } else if (sizeSlider.distanceToHandle(point) < Common.SIZE_HANDLE_RADIUS || 
    //         choppinessSlider.distanceToHandle(point) < Common.CHOPPINESS_HANDLE_RADIUS || 
    ...
    //         camera.changeAzimuth((mouseX - lastMouseX) / width * Common.SENSITIVITY);
    //         camera.changeElevation((mouseY - lastMouseY) / height * Common.SENSITIVITY);
    //         lastMouseX = mouseX;
    //         lastMouseY = mouseY;
    ...
    //         Common.setTransform(windDiv, 'translate3d(' + Common.WIND_SPEED_X + 'px, 0px, ' + Math.max(Common.MIN_WIND_SPEED_Z, windArrow.getTipZ() + WIND_SPEED_OFFSET) + 'px) rotateX(90deg)');
    //     } else if (mode === Common.SLIDING_SIZE) {
    //         sizeSlider.update(point[0], function (size) {
    //             simulator.setSize(size);
    //             Common.setText(sizeSpan, size, Common.SIZE_DECIMAL_PLACES);
    //         });
    ...
    //             Common.setText(choppinessDiv, choppiness, Common.CHOPPINESS_DECIMAL_PLACES);
    ...
    const onresize = function () {
        ...
        if (windowWidth / windowHeight > Common.MIN_ASPECT) {
            Common.makePerspectiveMatrix(projectionMatrix, Common.FOV, windowWidth / windowHeight, Common.NEAR, Common.FAR);
    ...
    const render = function render (currentTime) {
        ...
        const fovValue = 0.5 / Math.tan(Common.FOV / 2) * height;
        Common.setTransform(cameraDiv, 'translate3d(0px, 0px, ' + fovValue + 'px) ' + Common.toCSSMatrix(camera.getViewMatrix()) + ' translate3d(' + width / 2 + 'px, ' + height / 2 + 'px, 0px)');
...
if (Common.hasWebGLSupportWithExtensions(['OES_texture_float', 'OES_texture_float_linear'])) {
    main();
} else {
